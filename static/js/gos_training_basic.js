...
function unlockAudio() {
    ...
    soundPlayer.src = '/static/sounds/gos_basic/gos_task1.mp3';
...
function playSound(soundName) {
    ...
    soundPlayer.src = `/static/sounds/gos_basic/${soundName}`;
    soundPlayer.currentTime = 0;
...
window.completeTask = function(taskNum) {
    ...
    if (soundEnabled && taskNum === 1) {
        setTimeout(() => {
            playSound('gos_task2.mp3'); // При завершении 1 → звук 2
        }, 1000);
    }
};
...
window.toggleHelp = function(taskNum) {
    ...
    if (soundEnabled) {
        if (taskNum == 1) {
            playSound('gos_task1_help.mp3');
        } else if (taskNum == 2) {
            playSound('gos_task2_help.mp3');
        }
    }
};
